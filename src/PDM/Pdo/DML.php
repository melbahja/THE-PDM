...
class DML
{
     ...
     */
    public function selectOne($select, $from, $where = '', $fetch = \PDO::FETCH_ASSOC) 
    {
    ...
    */
    public function insert($into, array $array) 
    {
      ...
      $sql = "INSERT INTO {$this->prefix}{$into} ({$keys}) VALUE({$values})";

		if ( $this->connect->prepare($sql)->execute($data) === true ) {
	...
	*/
	public function multiInsert($into, array $array) 
	{
	      ...
	      $this->insert_ids[$key] = ( $this->insert($into, $value) === true ) ? $this->connect->lastInsertId() : false;
	  	}
	...
	*/
	public function update($table, $array, $where = '') 
	{
      ...
      $sql  = "UPDATE {$this->prefix}{$table} SET {$sql} {$where}"; 
